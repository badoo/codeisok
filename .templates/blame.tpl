...
{include file='header.tpl'}
 ...
 <div class="page_nav">
   ...
   <br />
   <a href="/?p={$project->GetProject()|urlencode}&amp;a=blob_plain&amp;h={$blob->GetHash()}&amp;f={$blob->GetPath()}">{t}plain{/t}</a> |
   {if $commit->GetHash() != $head->GetHash()}
     <a href="/?p={$project->GetProject()|urlencode}&amp;a=blame&amp;hb=HEAD&amp;f={$blob->GetPath()}">{t}HEAD{/t}</a>
   {else}
     {t}HEAD{/t}
   {/if}
    | blame
   ...
   {else}
	...
	{foreach from=$blob->GetData(true) item=blobline name=blob}
	  ...
	  <tr class="{$rowclass}">
	    <td class="date">
	      {if $blamecommit}
	        <a href="/?p={$project->GetProject()|urlencode}&amp;a=commit&amp;h={$blamecommit->GetHash()}" title="{$blamecommit->GetTitle()}" class="commitTip">{$blamecommit->GetAuthorEpoch()|date_format:"%Y-%m-%d %H:%M:%S"}</a>
	      {/if}
	    </td>
	    <td class="author">
	      {if $blamecommit}
