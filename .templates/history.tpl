...
<table class="git-table">
    ...
    {foreach from=$blob->GetHistory() item=historyitem}
        ...
        <tr>
            ...
            <td>
                <a href="/?p={$project->GetProject()|urlencode}&amp;a=commit&amp;h={$historycommit->GetHash()}" class="list commitTip" {if strlen($historycommit->GetTitle()) > 100}title="{$historycommit->GetTitle()}"{/if}><strong>{$historycommit->GetTitle(100)}</strong></a>
