...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	...
	bind: function(el) {
		...
		// We don't need to use auto scrolling or editor changes
	...
	_scrolling: function(editor_name) {
		...
		for (var name in this.editor) {
            ...
            var widgets_delta_this_side, widgets_delta_other_side;
            var previous_delta = {this_side: 0, other_side: 0};
				...
				if ((midline.line >= change[this_side+'-line-from'])) {
					...
					if (midline.line >= last_change[this_side+'-line-to']) {
						if (!change.hasOwnProperty(this_side+'-y-start') ||
                            ...
                            widgets_delta_this_side = this._widgetsHeightBeforeLine(this.editor['compare-'+this_side], change[this_side+'-line-to']);
                            widgets_delta_other_side = this._widgetsHeightBeforeLine(this.editor['compare-'+other_side], change[other_side+'-line-to']);
							top_adjust +=
								(change[this_side+'-y-end'] - change[this_side+'-y-start'] + widgets_delta_this_side - previous_delta['this_side']) -
								(change[other_side+'-y-end'] - change[other_side+'-y-start'] + widgets_delta_other_side - previous_delta['other_side']);
                            previous_delta = {this_side: widgets_delta_this_side, other_side: widgets_delta_other_side};
						}
					}
				}
			}
	...
	_draw_diff: function(editor_name1, editor_name2, changes) {
		...
		for (var i = 0; i < changes.length; ++i) {
			...
			// margin indicators
            var lhs_widgets_start = this._widgetsHeightBeforeLine(this.editor[editor_name1], change['lhs-line-from']);
            var lhs_widgets_end = this._widgetsHeightBeforeLine(this.editor[editor_name1], change['lhs-line-to']);
            var rhs_widgets_start = this._widgetsHeightBeforeLine(this.editor[editor_name2], change['rhs-line-from']);
            var rhs_widgets_end = this._widgetsHeightBeforeLine(this.editor[editor_name2], change['rhs-line-to']);

            var lhs_y_start = ((change['lhs-y-start'] + ex.lhs_scroller.scrollTop() + lhs_widgets_start) * ex.visible_page_ratio);
            var lhs_y_end = ((change['lhs-y-end'] + ex.lhs_scroller.scrollTop() + lhs_widgets_end) * ex.visible_page_ratio) + 1;
            var rhs_y_start = ((change['rhs-y-start'] + ex.rhs_scroller.scrollTop() + rhs_widgets_start) * ex.visible_page_ratio);
            var rhs_y_end = ((change['rhs-y-end'] + ex.rhs_scroller.scrollTop() + rhs_widgets_end) * ex.visible_page_ratio) + 1;
			...
			lhs_y_start = change['lhs-y-start'] + lhs_widgets_start;
			lhs_y_end = change['lhs-y-end'] + lhs_widgets_end;
			rhs_y_start = change['rhs-y-start'] + rhs_widgets_start;
			rhs_y_end = change['rhs-y-end'] + rhs_widgets_end;
    ...
    _widgetsHeightBeforeLine: function (editor, before_line) {
        var line_widgets;
        var all_widgets_height = 0;
        if (editor.lineCount() < before_line) {
            before_line = editor.lineCount() - 1;
        }
        for (var line_number = 0; line_number <= before_line; ++line_number) {
            line_widgets = editor.getLineHandle(line_number).widgets;
            if (line_widgets != undefined) {
                line_widgets.forEach(function (widget) {
                    all_widgets_height += $(widget.node).height();
                });
            }
        }
        return all_widgets_height;
    },
	trace: function(name) {
