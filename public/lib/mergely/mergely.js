...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	...
	_scrolling: function(editor_name) {
		...
		for (var name in this.editor) {
			...
			scroller.scrollTop(top_to).scrollLeft(left_to);
		}

		if (this.settings.autoupdate) {
			const self = this;

			this._autoupdateTimeout = setTimeout(function () {
				self._calculate_offsets(editor_name1, editor_name2, self.changes);
				self._markup_changes(editor_name1, editor_name2, self.changes);
				self._draw_diff(editor_name1, editor_name2, self.changes);
			}, this.settings.autoupdateDeferTime);
		}
	},
